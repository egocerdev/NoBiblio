...
function App() {
  ...
  const [publicProfileData, setPublicProfileData] = useState({ posts: [], comments: [] })
  ...
  async function loadComments(postId) {
    ...
    if (authorIds.length) {
      const { data: commentProfiles } = await supabase.from('profiles').select('*').in('id', authorIds)
      setProfiles((current) => ({ ...current, ...Object.fromEntries((commentProfiles || []).map((profile) => [profile.id, profile])) }))
    }
  ...
  async function addComment(postId, body, parentId = null) {
    if (!body.trim()) return
    if (!supabase) return
    const { error } = await supabase.from('comments').insert({ post_id: postId, author_id: session.user.id, parent_id: parentId, body: body.trim() })
    if (error) setSocialMessage(error.message); else await loadComments(postId)
  }
  async function openPublicProfile(profile) {
    setSelectedProfile(profile); setView('public-profile')
    if (!supabase) return
    const [{ data: profilePosts }, { data: profileComments }] = await Promise.all([
      supabase.from('posts').select('*').eq('author_id', profile.id).eq('status', 'published').order('published_at', { ascending: false }),
      supabase.from('comments').select('*').eq('author_id', profile.id).eq('status', 'visible').order('created_at', { ascending: false }),
    ])
    setPublicProfileData({ posts: profilePosts || [], comments: profileComments || [] })
  }
  function openPost(post) { setSelectedPost(post); setView('post-detail') }
  ...
  const visiblePosts = useMemo(() => posts.filter((post) => (!supabase || post.author_id === session?.user?.id) && (filter === 'all' || post.status === filter) && `${post.title} ${post.excerpt}`.toLocaleLowerCase('tr').includes(query.toLocaleLowerCase('tr'))), [posts, filter, query, session])
...
function Feed({ posts, profiles, following, comments, message, toggleFollow, loadComments, addComment, quotePost, reportTarget, rateAuthor, openPost }) {
  return <div className="social-page"><div className="page-heading"><div><span className="overline">Topluluk</span><h1>Akış</h1><p>Takip ettiklerinin ve topluluğun yeni yazıları.</p></div><span className="feed-note">{posts.length} yayın</span></div>{message && <div className="social-message">{message}</div>}<div className="feed-list">{posts.length ? posts.map((post) => <SocialPost key={post.id} post={post} profile={profiles[post.author_id]} profiles={profiles} isFollowing={following.has(post.author_id)} comments={comments[post.id] || []} toggleFollow={toggleFollow} loadComments={loadComments} addComment={addComment} quotePost={quotePost} reportTarget={reportTarget} rateAuthor={rateAuthor} openPost={openPost} />) : <div className="empty-list"><span>◌</span><h2>Akış henüz sessiz</h2><p>İlk yazılar yayınlandığında burada görünecek.</p></div>}</div></div>
}

function SocialPost({ post, profile, profiles, isFollowing, comments, toggleFollow, loadComments, addComment, quotePost, reportTarget, rateAuthor, openPost }) {
  ...
  async function submitComment() { if (comment.trim()) { await addComment(post.id, comment, replyTo); setComment(''); setReplyTo(null) } }
  return <article className="social-post social-preview-card" onClick={(event) => { if (!event.target.closest('button, input, textarea')) openPost(post) }}><div className="social-post-head"><div className="social-avatar">{profile?.avatar_url ? <img src={profile.avatar_url} alt="" /> : authorName.slice(0, 1).toUpperCase()}</div><div><strong>{authorName}</strong><small>{new Date(post.published_at || post.created_at).toLocaleDateString('tr-TR')} · {post.category}</small></div><button className="more-button" onClick={() => reportTarget({ post_id: post.id })}>•••</button></div><h2>{post.title}</h2>{post.cover_image && <img className="social-cover" src={post.cover_image} alt="" />}<p className="social-excerpt">{post.excerpt || 'Bu yazı için henüz özet eklenmemiş.'}</p><button className="read-preview" type="button" onClick={() => openPost(post)}>Yazıyı aç ↗</button><div className="social-actions"><button onClick={() => { setShowComments(!showComments); if (!showComments) loadComments(post.id) }}>◯ Yorumlar {comments.length ? `(${comments.length})` : ''}</button><button onClick={() => setShowQuote(!showQuote)}>❝ Alıntıla</button><button onClick={() => setShowRating(!showRating)}>☆ Puanla</button>{post.author_id && <button className={isFollowing ? 'following' : ''} onClick={() => toggleFollow(post.author_id)}>{isFollowing ? 'Takiptesin' : '+ Takip et'}</button>}</div>{showQuote && <div className="quote-editor"><span className="overline">Yeni alıntı</span><textarea value={quoteText} onChange={(event) => setQuoteText(event.target.value)} maxLength="1000" rows="3" placeholder="Alıntı metni" /><input value={quoteNote} onChange={(event) => setQuoteNote(event.target.value)} maxLength="1000" placeholder="Kendi yorumun (isteğe bağlı)" /><div><button className="secondary-button" type="button" onClick={() => setShowQuote(false)}>Vazgeç</button><button className="primary-button" type="button" onClick={() => { quotePost(post, quoteText, quoteNote); setShowQuote(false) }}>Alıntıyı paylaş ↗</button></div></div>}{showRating && <RatingForm authorId={post.author_id} rateAuthor={rateAuthor} />}{showComments && <div className="comments"><div className="comment-form">{replyTo && <span className="replying">Yanıtlanıyor <button type="button" onClick={() => setReplyTo(null)}>×</button></span>}<input value={comment} onChange={(event) => setComment(event.target.value)} placeholder={replyTo ? 'Bu yoruma yanıt yaz...' : 'Düşünceni yaz...'} onKeyDown={(event) => { if (event.key === 'Enter') submitComment() }} /><button type="button" onClick={submitComment}>Gönder</button></div><CommentThread comments={comments} profiles={profiles} parentId={null} onReply={setReplyTo} /></div>}</article>
}

function _SocialPostLegacy({ post, profile, profiles, isFollowing, comments, toggleFollow, loadComments, addComment, quotePost, reportTarget, rateAuthor, _openPost }) {
  const [comment, setComment] = useState('')
  const [replyTo, setReplyTo] = useState(null)
  const [showComments, setShowComments] = useState(false)
  const [showRating, setShowRating] = useState(false)
  ...
  const authorName = profile?.display_name || profile?.full_name || 'Adsız yazar'
  async function submitComment() { if (comment.trim()) { await addComment(post.id, comment, replyTo); setComment(''); setReplyTo(null) } }
  return <article className="social-post"><div className="social-post-head"><div className="social-avatar">{profile?.avatar_url ? <img src={profile.avatar_url} alt="" /> : authorName.slice(0, 1).toUpperCase()}</div><div><strong>{authorName}</strong><small>{new Date(post.published_at || post.created_at).toLocaleDateString('tr-TR')} · {post.category}</small></div><button className="more-button" onClick={() => reportTarget({ post_id: post.id })}>•••</button></div><h2>{post.title}</h2>{post.cover_image && <img className="social-cover" src={post.cover_image} alt="" />}<p className="social-excerpt">{post.excerpt}</p><div className="social-content" dangerouslySetInnerHTML={{ __html: post.content }} /><div className="social-actions"><button onClick={() => { setShowComments(!showComments); if (!showComments) loadComments(post.id) }}>◯ Yorumlar {comments.length ? `(${comments.length})` : ''}</button><button onClick={() => setShowQuote(!showQuote)}>❝ Alıntıla</button><button onClick={() => setShowRating(!showRating)}>☆ Puanla</button>{post.author_id && <button className={isFollowing ? 'following' : ''} onClick={() => toggleFollow(post.author_id)}>{isFollowing ? 'Takiptesin' : '+ Takip et'}</button>}</div>{showQuote && <div className="quote-editor"><span className="overline">Yeni alıntı</span><textarea value={quoteText} onChange={(event) => setQuoteText(event.target.value)} maxLength="1000" rows="3" placeholder="Alıntı metni" /><input value={quoteNote} onChange={(event) => setQuoteNote(event.target.value)} maxLength="1000" placeholder="Kendi yorumun (isteğe bağlı)" /><div><button className="secondary-button" type="button" onClick={() => setShowQuote(false)}>Vazgeç</button><button className="primary-button" type="button" onClick={() => { quotePost(post, quoteText, quoteNote); setShowQuote(false) }}>Alıntıyı paylaş ↗</button></div></div>}{showRating && <RatingForm authorId={post.author_id} rateAuthor={rateAuthor} />}{showComments && <div className="comments"><div className="comment-form">{replyTo && <span className="replying">Yanıtlanıyor <button type="button" onClick={() => setReplyTo(null)}>×</button></span>}<input value={comment} onChange={(event) => setComment(event.target.value)} placeholder={replyTo ? 'Bu yoruma yanıt yaz...' : 'Düşünceni yaz...'} onKeyDown={(event) => { if (event.key === 'Enter') submitComment() }} /><button type="button" onClick={submitComment}>Gönder</button></div><CommentThread comments={comments} profiles={profiles} parentId={null} onReply={(commentId) => setReplyTo(commentId)} /></div>}</article>
}

function CommentThread({ comments, profiles, parentId, onReply }) { return <div className={parentId ? 'comment-children' : ''}>{comments.filter((comment) => comment.parent_id === parentId).map((item) => { const author = profiles[item.author_id]?.display_name || profiles[item.author_id]?.full_name || 'NoBiblio yazarı'; return <div className="comment" key={item.id}><span className="comment-avatar">{profiles[item.author_id]?.avatar_url ? <img src={profiles[item.author_id].avatar_url} alt="" /> : author.slice(0, 1).toUpperCase()}</span><div><strong className="comment-author">{author}</strong><p>{item.body}</p><small>{new Date(item.created_at).toLocaleDateString('tr-TR')} · <button type="button" onClick={() => onReply(item.id)}>Yanıtla</button></small><CommentThread comments={comments} profiles={profiles} parentId={item.id} onReply={onReply} /></div></div> })}</div> }
...
function Profile({ form, setForm, save, message, followingProfiles = [], followerProfiles = [], openProfile }) { return <div className="profile-page"><div className="page-heading"><div><span className="overline">Sosyal ayarlar</span><h1>Profilim</h1><p>İnsanların seni tanıması için alanını düzenle.</p></div></div><div className="connection-stats"><div><strong>{followingProfiles.length}</strong><span>Takip ettiklerim</span></div><div><strong>{followerProfiles.length}</strong><span>Takipçilerim</span></div></div><form className="profile-form" onSubmit={save}><div className="profile-preview"><div className="large-avatar">{form.avatar_url ? <img src={form.avatar_url} alt="" /> : (form.display_name || 'N').slice(0, 1).toUpperCase()}</div><strong>{form.display_name || 'İsmin'}</strong><p>{form.bio || 'Bio alanın burada görünecek.'}</p></div><label>Görünen ad<input value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} placeholder="Adın veya kullanıcı adın" required /></label><label>Profil fotoğrafı<input type="file" accept="image/*" onChange={(event) => uploadHandlers.avatar?.(event.target.files?.[0])} />{form.avatar_url && <small className="upload-done">Profil fotoğrafı yüklendi.</small>}</label><label>Bio<textarea value={form.bio} onChange={(event) => setForm({ ...form, bio: event.target.value })} maxLength="280" rows="4" placeholder="Kendinden kısaca bahset..." /></label><label>Web sitesi<input value={form.website_url} onChange={(event) => setForm({ ...form, website_url: event.target.value })} placeholder="https://..." /></label><button className="primary-button" type="submit">Profili kaydet <span>↗</span></button>{message && <p className="form-message">{message}</p>}</form><div className="connection-lists"><ConnectionList title="Takip ettiklerim" profiles={followingProfiles} empty="Henüz kimseyi takip etmiyorsun." openProfile={openProfile} /><ConnectionList title="Takipçilerim" profiles={followerProfiles} empty="Henüz takipçin yok." openProfile={openProfile} /></div></div> }

function ConnectionList({ title, profiles, empty, openProfile }) { return <section className="connection-list"><h2>{title}</h2>{profiles.length ? profiles.map((profile) => <button className="connection-person" type="button" key={profile.id} onClick={() => openProfile?.(profile)}><div className="comment-avatar">{profile.avatar_url ? <img src={profile.avatar_url} alt="" /> : (profile.display_name || profile.full_name || 'N').slice(0, 1).toUpperCase()}</div><span>{profile.display_name || profile.full_name || 'Adsız yazar'}<small>{profile.bio || 'NoBiblio yazarı'}</small></span><b>↗</b></button>) : <p>{empty}</p>}</section> }

function PublicProfile({ profile, posts, comments, back }) { const name = profile?.display_name || profile?.full_name || 'Adsız yazar'; return <div className="public-profile"><button className="back-button" type="button" onClick={back}>← Profilime dön</button><header className="public-profile-head"><div className="large-avatar">{profile?.avatar_url ? <img src={profile.avatar_url} alt="" /> : name.slice(0, 1).toUpperCase()}</div><div><span className="overline">Yazar profili</span><h1>{name}</h1><p>{profile?.bio || 'Bu yazar henüz bio eklemedi.'}</p></div></header><div className="public-profile-grid"><section><h2>Yazıları</h2>{posts.length ? posts.map((post) => <article className="public-post" key={post.id}><span className="overline">{post.category}</span><h3>{post.title}</h3><p>{post.excerpt}</p><small>{new Date(post.published_at || post.created_at).toLocaleDateString('tr-TR')}</small></article>) : <p>Henüz yayınlanmış yazı yok.</p>}</section><section><h2>Yorumları</h2>{comments.length ? comments.map((comment) => <article className="public-comment" key={comment.id}><p>{comment.body}</p><small>{new Date(comment.created_at).toLocaleDateString('tr-TR')}</small></article>) : <p>Henüz yorum yok.</p>}</section></div></div> }

function PostDetail({ post, profile, back }) { const author = profile?.display_name || profile?.full_name || 'Adsız yazar'; return <article className="post-detail"><button className="back-button" type="button" onClick={back}>← Akışa dön</button><span className="overline">{post?.category || 'Genel'}</span><h1>{post?.title || 'Başlıksız yazı'}</h1><div className="post-detail-meta"><span className="social-avatar">{profile?.avatar_url ? <img src={profile.avatar_url} alt="" /> : author.slice(0, 1).toUpperCase()}</span><span>{author}<small>{post && new Date(post.published_at || post.created_at).toLocaleDateString('tr-TR')}</small></span></div>{post?.cover_image && <img className="post-detail-cover" src={post.cover_image} alt="" />}<p className="post-detail-excerpt">{post?.excerpt}</p><div className="post-detail-content" dangerouslySetInnerHTML={{ __html: post?.content || '<p>Bu yazının içeriği boş.</p>' }} /></article> }
