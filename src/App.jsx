...
function Editor({ post, update, save, message, back }) {
...
function Profile({ form, setForm, save, message }) { return <div className="profile-page"><div className="page-heading"><div><span className="overline">Sosyal ayarlar</span><h1>Profilim</h1><p>İnsanların seni tanıması için alanını düzenle.</p></div></div><form className="profile-form" onSubmit={save}><div className="profile-preview"><div className="large-avatar">{form.avatar_url ? <img src={form.avatar_url} alt="" /> : (form.display_name || 'N').slice(0, 1).toUpperCase()}</div><strong>{form.display_name || 'İsmin'}</strong><p>{form.bio || 'Bio alanın burada görünecek.'}</p></div><label>Görünen ad<input value={form.display_name} onChange={(event) => setForm({ ...form, display_name: event.target.value })} placeholder="Adın veya kullanıcı adın" required /></label><label>Profil fotoğrafı<input type="file" accept="image/*" onChange={(event) => uploadHandlers.avatar?.(event.target.files?.[0])} />{form.avatar_url && <small className="upload-done">Profil fotoğrafı yüklendi.</small>}</label><label>Bio<textarea value={form.bio} onChange={(event) => setForm({ ...form, bio: event.target.value })} maxLength="280" rows="4" placeholder="Kendinden kısaca bahset..." /></label><label>Web sitesi<input value={form.website_url} onChange={(event) => setForm({ ...form, website_url: event.target.value })} placeholder="https://..." /></label><button className="primary-button" type="submit">Profili kaydet <span>↗</span></button>{message && <p className="form-message">{message}</p>}</form></div> }
